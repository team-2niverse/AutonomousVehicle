...
public:
  ...
  : Node("lkas_test_node"), latest_speed_(0.0f), mode_(Mode::MANUAL)
  {
    ...
    // TFLite 모델 로드
    std::string model_path = "/root/mypilot2.tflite";
    ...
    if (interpreter_->inputs().size() != 3) {
        RCLCPP_FATAL(this->get_logger(), "모델 입력이 3개가 아닙니다.");
        rclcpp::shutdown();
        return;
    }
    //shape info function
    ...
    //input (vector>
    vec_input_tensor_ = interpreter_->input_tensor(0);

    //input (velocity)
    vel_input_tensor_ = interpreter_->input_tensor(1); // 1번 인덱스
    ...
    RCLCPP_INFO(this->get_logger(), "모델 입력 1번: 모양=%s, 타입=%s",
                input_0_shape.c_str(),
                (vel_input_type_ == kTfLiteFloat32 ? "Float32" : (vel_input_type_ == kTfLiteUInt8 ? "UInt8" : "Unknown")));
    
    //input (image)
    input_tensor_ = interpreter_->input_tensor(2);
    if (input_tensor_ == nullptr) {
        RCLCPP_FATAL(this->get_logger(), "입력 텐서 2 (이미지)을 가져올 수 없습니다.");
        rclcpp::shutdown();
        return;
    }
    ...
    RCLCPP_INFO(this->get_logger(), "모델 입력 텐서 2 크기: [H=%d, W=%d, C=%d]", input_height_, input_width_, input_channels_);
    RCLCPP_INFO(this->get_logger(), "모델 입력 텐서2 타입: %s", (input_type_ == kTfLiteFloat32 ? "Float32" : (input_type_ == kTfLiteUInt8 ? "UInt8" : "Unknown")));
...
private:
  ...
  //void image_callback(const sensor_msgs::msg::CompressedImage::SharedPtr msg)
  {
    ...
    float* vec_input_data_ptr = interpreter_->typed_input_tensor<float>(0);
    vec_input_data_ptr[0] = 1.0f;
    vec_input_data_ptr[1] = 0.0f;

    if (input_type_ == kTfLiteUInt8) {
      // --- UInt8 (양자화) 모델 ---
      uint8_t* input_data_ptr = interpreter_->typed_input_tensor<uint8_t>(2);
      ...
      float* input_data_ptr = interpreter_->typed_input_tensor<float>(2);
      if (input_data_ptr == nullptr) {
    ...
    if (vel_input_type_ == kTfLiteFloat32) {
        float* vel_input_data_ptr = interpreter_->typed_input_tensor<float>(1);
        ...
        uint8_t* vel_input_data_ptr = interpreter_->typed_input_tensor<uint8_t>(1);
        if (vel_input_data_ptr == nullptr) {
  ...
  TfLiteTensor* vec_input_tensor_ = nullptr;
